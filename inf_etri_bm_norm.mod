...
s.t. dinf{i in ALTS, j in CRIT}:
	d_inf[i,j] >= perfs[i,j]/(gmax[j]+1) - gb[assign[i]-1,j] + epsilon;

s.t. dsup{i in ALTS, j in CRIT}:
	d_sup[i,j] >= perfs[i,j]/(gmax[j]+1) - gb[assign[i],j] + epsilon;

s.t. dinf2{i in ALTS, j in CRIT}:
	d_inf[i,j] <= perfs[i,j]/(gmax[j]+1) - gb[assign[i]-1,j] + 1;

s.t. dsup2{i in ALTS, j in CRIT}:
	d_sup[i,j] <= perfs[i,j]/(gmax[j]+1) - gb[assign[i],j] + 1;
